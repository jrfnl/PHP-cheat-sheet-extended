...
function fix_content( $content ) {
	...
	$regex_search = array(
		...
		12 => '`<a href="http[s]?://([a-z\.-]+)/(arithmetic|compare|test)/" class="top-link(?: top-active)?">`',
		// Make sure any potential links to php.net are properly linked.
		13 => '`\[(http[s]?://php\.net/manual/([^\]]+))\]`',
	...
	$regex_replace = array(
		0  => '<!DOCTYPE html PUBLIC',
		5  => '<a href=$1https://php.net/function.',
		7  => '<th>array()<br />					</th>',
		8  => 'array()<br />',
		9  => '<t$1$2>array()<br /></t$1>',
		10 => '<option value="live">PHP 5.6.40</option>', // IMPORTANT! Change this if the PHP version on the server changes!!
		12 => '<a href="https://$1/index.php?page=$2&amp;phpversion=php' . PHP_VERSION . '" class="top-link$3">',
		13 => '[<a href="$1">$2</a>]',
		14 => '<a href="$1"',
	);
	...
	if ( $GLOBALS['verbose'] !== 2 ) {
		...
		if ( PHP_VERSION_ID >= 50100 ) {
			foreach ( $regex_search as $key => $regex ) {
				// phpcs:ignore PHPCompatibility.FunctionUse.NewFunctionParameters.preg_replace_countFound -- wrapped in version check.
